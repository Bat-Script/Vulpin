        ...
        while ip < len(lines):
            ...
            try:
                ...
                if line.startswith('I'):
                    m_old = re.match(r'I"([^"]*)"\s*=\s*(.*)', line)
                    if m_old:
                        var_name = m_old.group(1); expr_str = m_old.group(2).strip()
                        val = evaluator.eval(expr_str); ctx.set_var(var_name, val); ip += 1; continue
                    raise VulError(f"Invalid old-style assignment: {line}", line=self.current_line, tip="Use: I\"var\"=expr or var=expr")
